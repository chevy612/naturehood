...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (query === null) return socialFailure('q is required', 400)
    if (!query.trim()) return socialJson(ok(feedResponse([], null)))
    ...
    const nextCursor = hasNext && last
      ? encodeSearchCursor({ rank: last.search_rank ?? 0, createdAt: last.created_at, id: last.id })
      : null
